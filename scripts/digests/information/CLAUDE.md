...
- Dedup: SHA-256 hashing with JSON cache, 7-day rolling expiry (hash → timestamp), capped at 10,000 entries
- RSS fetching: parallel via `ThreadPoolExecutor` (8 workers, 15s timeout per feed)
- HTML escaping: all RSS-sourced content sanitized with `html.escape()` before email output
- JSON extraction: regex-based (`JSON_ARRAY_RE`) with explicit `JSONDecodeError` handling
- Scheduling: launchd — Monday & Friday at 6:00 AM (fires on wake if missed), delivery by 6:30 AM
...
## File Sync
- Files exist in two locations: this repo (`~/intelligence-dashboard/scripts/digests/information/`) and Desktop (`~/Desktop/_Claude_AI_docs/_Projects/AI_Product_Development/Building Your Virtual Team of 50/05 Information Digest/`)
...
## Gotchas
- `.env` permission warning fires at startup if group/other-readable
- `__version__ = "3.1"` constant — referenced in HTML footer and JSON export
