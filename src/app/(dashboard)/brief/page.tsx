            ...
            <thead>
              <tr style={{ background: 'var(--bg)' }}>
                {headerCells.map((cell, ci) => (
                  <th key={ci} className="text-left px-4 py-2.5 font-medium text-xs uppercase tracking-wider" style={{ color: 'var(--text-muted)' }}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
              ...
              {briefs.map((brief) => (
                <button
                  ...
                  style={{
                    ...
                    color: selectedBrief?.id === brief.id ? '#ffffff' : 'var(--text)',
                  }}
                >
                  ...
                  <p className="text-xs mt-0.5" style={{ color: selectedBrief?.id === brief.id ? 'var(--text-sidebar)' : 'var(--text-muted)' }}>
                    Generated by {brief.generated_by}
                  </p>
                </button>
              ))}
              {briefs.length === 0 && (
                <p className="text-xs" style={{ color: 'var(--text-muted)' }}>No briefs generated yet.</p>
              )}
            </div>
          </div>

          {/* Brief Trend */}
          <div className="rounded-lg border p-4 mt-4" style={{ background: 'var(--card-bg)', borderColor: 'var(--border)' }}>
        ...
        <div className="lg:col-span-3 order-1 lg:order-2">
          {selectedBrief ? (
                      ...
                      return (
                        <>
                          ...
                          <div className="p-3 rounded-lg" style={{ background: 'var(--bg)' }}>
                            ...
                            <p className="text-xs mt-1" style={{ color: 'var(--text-muted)' }}>Next Deadline</p>
                          </div>
                        </>
                      );
                    })()}
                ...
                <div className="rounded-lg border p-5" style={{ background: 'var(--card-bg)', borderColor: 'var(--border)' }}>
                  ...
                  <div className="flex flex-wrap gap-3">
                    {Object.entries(selectedBrief.competitor_mentions as Record<string, number>)
                      .sort(([, a], [, b]) => b - a)
                      .map(([name, count]) => (
                        <div key={name} className="flex items-center gap-2 px-3 py-1.5 rounded-lg" style={{ background: 'var(--bg)' }}>
            ...
            <div className="rounded-lg border p-16 text-center" style={{ background: 'var(--card-bg)', borderColor: 'var(--border)' }}>
