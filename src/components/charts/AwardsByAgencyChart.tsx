...
export function AwardsByAgencyChart({ awards }: { awards: Award[] }) {
  ...
  if (data.length === 0) {
    return <p className="text-sm text-center py-8" style={{ color: 'var(--text-secondary)' }}>No award data yet.</p>;
  }
  ...
  return (
    <ResponsiveContainer width="100%" height={250}>
      <BarChart data={data} layout="vertical" margin={{ left: 20 }}>
        ...
        <YAxis dataKey="agency" type="category" width={140} tick={{ fontSize: 11, fill: 'var(--text-secondary)' }} />
        <Tooltip
